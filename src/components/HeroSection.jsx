...
export default function HeroSection({
  ...
  // Loading screen: hide when intro is ready + slot measured, min 1.2s, max 2.5s
  useEffect(() => {
    if (introPlayed || prefersReducedMotion) {
    ...
    // Hide when introReady + slotRect are both available, with brief delay
    if (introReady && slotRect) {
      const timer = setTimeout(() => setShowLoading(false), 350);
      return () => clearTimeout(timer);
    }
  }, [introReady, slotRect, introPlayed, prefersReducedMotion]);

  // Hard max timeout for loading screen (2.5s)
  useEffect(() => {
        ...
        {showLoading && (
          ...
          >
            <div className="flex flex-col items-center space-y-5">
              <span className="font-serif text-5xl tracking-[0.2em] text-primary font-light">Æ</span>
              <span className="font-sans text-[9px] tracking-[0.4em] text-on-surface/40 uppercase">AETHERIS</span>
              <div className="w-16 h-[1px] bg-primary/20 mt-2 overflow-hidden rounded-full">
                <motion.div
